...
def _get_tenant_name():
    ...
    blueprint_from_rest = client.blueprints.get(ctx.blueprint.id)
    if blueprint_from_rest['visibility'] == VisibilityState.GLOBAL:
        return blueprint_from_rest['tenant_name']
    return ctx.tenant_name


def _get_deployment_blueprint():
    try:
        client = get_rest_client()
        dep_upd = client.deployment_updates.list(
            deployment_id=ctx.deployment.id,
            sort='created_at')[-1]
        return client.deployment_updates.get(dep_upd.id)['new_blueprint_id']
    except KeyError:
        raise NonRecoverableError(
            'Can\'t get blueprint for deployment {dep}'.format(
...
def get_deployment_file_path(filename):
    file_path = os.path.join(get_node_instance_dir(), filename)
    ctx.logger.debug('Value file_path is {loc}.'.format(
        loc=file_path))
    return file_path
